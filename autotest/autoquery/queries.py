...
class Query:
    ...
    def login(self, username="fdse_microservice", password="111111") -> bool:
        ...
        r = self.session.post(url=url, headers=headers, json=data, verify=False)
    ...
    def query_advanced_ticket(
        self, place_pair: tuple = (), type: str = "cheapest", date: str = "", headers: dict = {}
    ) -> List[str]:
        """
        高级查询
    ...
    def query_orders(
        self, types: tuple = tuple([0, 1]), query_other: bool = False, headers: dict = {}
    ) -> List[tuple]:
        """
    ...
    def put_consign(self, result, headers: dict = {}) -> str:
        ...
        if res.status_code == 200 or res.status_code == 201:
            ...
            logger.warning(f"order {order_id} failed, code: {res.status_code}, text: {res.text}")
            return None
    ...
    def query_admin_travel(self, headers: dict = {}):
        ...
        if r.status_code == 200 and r.json()["status"] == 1:
            ...
            logger.warning(f"faild to query admin travel with status_code: {r.status_code}")
        return
    ...
    def preserve(
        ...
        # print(f"contacts_result:{contacts_result}")
        ...
        res = self.session.post(url=PRESERVE_URL, headers=headers, json=base_preserve_payload)
