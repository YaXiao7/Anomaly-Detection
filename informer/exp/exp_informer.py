...
class Exp_Informer(Exp_Basic):
    ...
    def predict(self, params, end=0):
        self.load_model(params)
        pred_data, pred_loader = self._get_data(flag="pred", end=end)
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(pred_loader):
            ...
            preds.append(pred.detach().cpu().numpy())
            save_batch_y.append(true.detach().cpu().numpy())
        ...
        # np.save(folder_path + "batchy.npy", save_batch_y)

        return preds, save_batch_y
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        batch_y_mark = batch_y_mark.float().to(self.device)  # 放入cuda

        # decoder input
        if self.args.padding == 0:
            dec_inp = torch.zeros([batch_y.shape[0], self.args.pred_len, batch_y.shape[-1]]).float()  # 24个0
        elif self.args.padding == 1:
            dec_inp = torch.ones([batch_y.shape[0], self.args.pred_len, batch_y.shape[-1]]).float()
        dec_inp = (
            torch.cat([batch_y[:, : self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
        )  # 48个值拼接上24个0
        # encoder - decoder
        if self.args.use_amp:
